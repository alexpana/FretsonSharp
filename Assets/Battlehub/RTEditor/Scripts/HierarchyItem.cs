    ...
    public class HierarchyItem : MonoBehaviour
    {
        ...
        private void OnTransformParentChanged()
        {
            ...
            if (transform.parent != m_parentTransform)
            {
                ...
                var parentExp = CreateChainToParent(transform.parent);
        ...
        private ExposeToEditor CreateChainToParent(Transform parent)
        {
            ...
            if (parent != null) parentExp = parent.GetComponentInParent<ExposeToEditor>();
            if (parentExp == null) return null;
        ...
        private void TryDestroyChainToParent(Transform parent, ExposeToEditor parentExp)
        {
            ...
            while (parent != null && parent.gameObject != parentExp.gameObject)
            {
                if (!parent.GetComponent<ExposeToEditor>())
                {
                    var hierarchyItem = parent.GetComponent<HierarchyItem>();
                    if (hierarchyItem)
        ...
        {
            var childrenCount = parentTransform.childCount;
            if (childrenCount == 0) return false;

            for (var i = 0; i < childrenCount; ++i)
            {
                var childTransform = parentTransform.GetChild(i);
                var child = childTransform.GetComponent<ExposeToEditor>();
                if (child != null) return true;
                var hierarchyItem = childTransform.GetComponent<HierarchyItem>();
        ...
        private void UpdateChildren(Transform parentTransform, ExposeToEditor parentExp)
        {
            ...
            for (var i = 0; i < childrenCount; ++i)
            {
                var childTransform = parentTransform.GetChild(i);
                var child = childTransform.GetComponent<ExposeToEditor>();
                var childHierarcyItem = childTransform.GetComponent<HierarchyItem>();
                if (child != null)
                {
                ...
                {
                    if (childHierarcyItem != null) UpdateChildren(childTransform, parentExp);
                }
            }
        }
    }
