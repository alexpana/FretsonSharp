...
namespace Battlehub.SplineEditor
{
    ...
    public class SplineControlPointEditor : SplineBaseEditor
    {
        ...
        {
            InitActiveSpline();
            var controlPoint = (SplineControlPoint)target;
            ...
            if (EditorWindow.focusedWindow == SceneView.lastActiveSceneView)
                ...
                {
                    var controlPoints = m_spline.GetSplineControlPoints();
                    for (var i = 0; i < controlPoints.Length; ++i) controlPoints[i].UpdateAngle();
                }
        }
        ...
        protected override void OnInspectorGUIOverride()
        {
            ...
            if (ConvergingSpline)
            {
                if (GUILayout.Button("Cancel")) ConvergingSpline = null;
                return;
            }
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("OUT -> Branch")) CreateBranch(m_spline, SelectedIndex, false);
                ...
                if (GUILayout.Button("Insert"))
                {
                    ...
                    Selection.activeGameObject =
                        m_spline.GetSplineControlPoints().ElementAt(SelectedIndex + 3).gameObject;
                }
                ...
                if (GUILayout.Button("Branch -> IN")) CreateBranch(m_spline, SelectedIndex, true);
            ...
            if ((SelectedIndex == 0 && m_spline.PrevSpline == null) ||
                ...
                if (!m_spline.HasBranches(SelectedIndex))
                    ...
                    {
                        if (m_spline.Loop)
                            EditorUtility.DisplayDialog("Unable to converge",
                                "Unable to converge. Selected spline has loop.", "OK");
                        else
        ...
        protected override SplineBase GetTarget()
        {
            ...
            if (controlPoint)
            {
                var spline = controlPoint.GetComponentInParent<SplineBase>();
                return spline;
            }
        ...
        {
            var branch = SplineEditor.CreateSpline(
        ...
        {
            var curveIndex = Mathf.Min((controlPointIndex + 1) / 3, spline.CurveCount - 1);
            RecordHierarchy(spline.Root, "Battlehub.Spline.Remove");
            if (!spline.Remove(curveIndex))
                EditorUtility.DisplayDialog("Action cancelled",
                    "Unable to remove last curve in spline. If you want to remove whole spline or branch, please select it in hierarchy and press delete button",
                    "OK");
            else
                EditorUtility.SetDirty(spline);
        }
