...
{
    public class MessageChannel<T> : IMessageChannel<T> where T : IMessage
    {
        private readonly List<Action<T>> m_MessageHandlers = new();

        /// <summary>
        ///     This queue of actions that would either add or remove subscriber is used to prevent problems from immediate
        ///     modification
        ...
        {
            Assert.IsTrue(!m_MessageHandlers.Contains(handler),
            ...
            {
                if (h != null && !m_MessageHandlers.Contains(h)) m_MessageHandlers.Add(h);
            }
        }
