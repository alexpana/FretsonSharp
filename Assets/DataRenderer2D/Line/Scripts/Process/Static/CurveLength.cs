...
namespace geniikw.DataRenderer2D
{
    public static class CurveLength
    {
        ...
        /// </summary>
        private static float Quadratic(Vector3 p0, Vector3 cp, Vector3 p1)
        {
            if (p0 == p1)
                if (p0 == cp)
                    return 0f;
            ...
            var l1 = q / (8f * Mathf.Pow(c, 1.5f)) * (Mathf.Log(2.0f * Mathf.Sqrt(c * sumCBA) + twoCpB) -
                                                      Mathf.Log(2.0f * Mathf.Sqrt(c * a) + b));
            return l0 + l1;
        }

        private static float QuadraticApproximation(Vector3 p0, Vector3 c0, Vector3 c1, Vector3 p1)
        {
