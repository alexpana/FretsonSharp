...
namespace geniikw.DataRenderer2D.Hole
{
    ...
    {
        private readonly IHole m_hole;
        private readonly IUnitSize m_unit;
        ...
        {
            var hole = m_hole.Hole;
            var color = hole.Color;
            Vector3 half = m_unit.Size / 2f;
            var p0 = new Vertex(Vector3.zero - half, Vector2.zero, color);
            var p1 = new Vertex(Vector3.right * m_unit.Size.x - half, Vector2.right, color);
            var p2 = new Vertex(Vector3.up * m_unit.Size.y - half, Vector2.up, color);
            var p3 = new Vertex(half, Vector2.one, color);

            var outer = new[] { p0, p1, p2, p3 };
            var inner = new Vertex[hole.Inner];

            var unit = Mathf.PI * 2f / hole.Inner;
            var angle = hole.Angle * Mathf.Deg2Rad;
            for (var i = 0; i < hole.Inner; i++)
            {
