        ...
        public IEnumerable<IMesh> Draw()
        {
            ...
            for (float i = 0; i < count; i++)
            {
                ...
                if (!startFlag)
                {
                    ...
                    Intersect.ClosestPointsOnTwoLines(out aOut, out useless, Vector3.one * 0.5f,
                        a.uv - Vector2.one * 0.5f, aUV, uvV);
                    Intersect.ClosestPointsOnTwoLines(out bOut, out useless, Vector3.one * 0.5f,
                        a.uv - Vector2.one * 0.5f, bUV, uvV);
                    a.uv = aOut;
                    d.uv = bOut;
                }
                ...
                if (!endFlag && endAngle < a1)
                {
                    ...
                    Intersect.ClosestPointsOnTwoLines(out aOut, out useless, Vector2.one * 0.5f,
                        b.uv - Vector2.one * 0.5f, aUV, uvV);
                    Intersect.ClosestPointsOnTwoLines(out bOut, out useless, Vector2.one * 0.5f,
                        b.uv - Vector2.one * 0.5f, bUV, uvV);
                    b.uv = aOut;
                    c.uv = bOut;
                }
