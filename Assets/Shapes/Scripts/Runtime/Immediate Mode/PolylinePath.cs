...
namespace Shapes
{
    public class PolylinePath : PointPath<PolylinePoint>
    {
        ...
        private bool lastUsedClosed = false;
        ...
        {
            if (meshDirty == false) // polyline itself didn't change, but the render state might force us to update
        ...
        private void TryUpdateMesh(bool closed, PolylineJoins joins)
        {
            ...
            ShapesMeshGen.GenPolylineMesh(mesh, path, closed, joins, false, true);
        }
        ...
        {
            AddPoints(pts.Select(point => new PolylinePoint(point, Color.white)));
        }
        ...
        {
            AddPoints(pts.Select(point => new PolylinePoint(point, color)));
        }
        ...
        {
            AddPoints(pts.Zip(colors, (p, c) => new PolylinePoint(p, c)));
        }
        ...
        {
            AddPoints(pts.Zip(thicknesses, (p, t) => new PolylinePoint(p, Color.white, t)));
        }
        ...
        {
            AddPoints(pts.Zip(colors, thicknesses, (p, c, t) => new PolylinePoint(p, c, t)));
        }
        ...
        public void BezierTo(Vector3 startTangent, Vector3 endTangent, Vector3 end, float pointsPerTurn)
        {
            ...
            var pointCount = CalcBezierPointCount(LastPoint.point, startTangent, endTangent, end, pointsPerTurn);
        ...
        {
            if (CheckCanAddContinuePoint()) return;
            AddPoints(ShapesMath.CubicBezierPointsSkipFirstMatchStyle(LastPoint, LastPoint.point, startTangent,
                endTangent, end, pointCount));
        }

        // POLYLINEPOINT ENDPOINTS:
        /// <summary>
        ///     A cubic bezier curve, using the previous point as the starting point. Color and thickness etc. will blend
        ///     toward the end point values
        /// </summary>
        [MethodImpl(INLINE)]
        ...
        /// <summary>
        ///     A cubic bezier curve, using the previous point as the starting point. Number of points is given by density in
        ///     number of points per full 360° turn. Color and thickness etc. will blend toward the end point values
        /// </summary>
        public void BezierTo(Vector3 startTangent, Vector3 endTangent, PolylinePoint end, float pointsPerTurn)
        {
            if (CheckCanAddContinuePoint()) return;
            var pointCount = CalcBezierPointCount(LastPoint.point, startTangent, endTangent, end.point, pointsPerTurn);
            BezierTo(startTangent, endTangent, end, pointCount);
        }
        ...
        {
            if (CheckCanAddContinuePoint()) return;
            var ppB = PolylinePoint.Lerp(LastPoint, end, 1f / 3f); // blend all other properties, assume thirds
            ppB.point = startTangent;
            var ppC = PolylinePoint.Lerp(LastPoint, end, 2f / 3f);
        ...
        /// <summary>
        ///     A cubic bezier curve, using the previous point as the starting point. Number of points is given by density in
        ///     number of points per full 360° turn. Color and thickness etc. will blend across the point values
        /// </summary>
        public void BezierTo(PolylinePoint startTangent, PolylinePoint endTangent, PolylinePoint end,
            float pointsPerTurn)
        {
            if (CheckCanAddContinuePoint()) return;
            var pointCount = CalcBezierPointCount(LastPoint.point, startTangent.point, endTangent.point, end.point,
                pointsPerTurn);
            BezierTo(startTangent, endTangent, end, pointCount);
        }

        /// <summary>
        ///     Adds points of a cubic bezier curve, using the previous point as the starting point. Color and thickness etc.
        ///     will blend across the point values
        /// </summary>
        ...
        private static int CalcBezierPointCount(Vector3 a, Vector3 b, Vector3 c, Vector3 d, float pointsPerTurn)
        {
            ...
            var curveSumDeg = ShapesMath.GetApproximateCurveSum(a, b, c, d, sampleCount);
            var angSpanTurns = curveSumDeg / 360f;
            return Mathf.Max(2, Mathf.RoundToInt(angSpanTurns * pointsPerTurn));
        }

        /// <summary>
        ///     Adds points of an arc wedged into the corner defined by the previous point, corner, and next, with the given
        ///     point count
        /// </summary>
        [MethodImpl(INLINE)]
        ...
        /// <summary>
        ///     Adds points of an arc wedged into the corner defined by the previous point, corner, and next, with the given
        ///     point count. Color and thickness etc. will blend from start to end
        /// </summary>
        [MethodImpl(INLINE)]
        ...
        /// <summary>
        ///     Adds points of an arc wedged into the corner defined by the previous point, corner, and next. Color and
        ///     thickness etc. will blend from start to end
        /// </summary>
        [MethodImpl(INLINE)]
        ...
        /// <summary>
        ///     Adds points of an arc wedged into the corner defined by the previous point, corner, and next, with the given
        ///     point density in number of points per full 360° turn
        /// </summary>
        [MethodImpl(INLINE)]
        ...
        /// <summary>
        ///     Adds points of an arc wedged into the corner defined by the previous point, corner, and next, with the given
        ///     point density in number of points per full 360° turn. Color and thickness etc. will blend from start to end
        /// </summary>
        [MethodImpl(INLINE)]
        ...
        private void AddArcPoints(Vector3 corner, Vector3 next, float radius, bool useDensity, int targetPointCount,
            float pointsPerTurn)
        {
            if (CheckCanAddContinuePoint()) return;
            var ppNext = LastPoint;
            ppNext.point = next;
            AddArcPoints(corner, ppNext, radius, useDensity, targetPointCount, pointsPerTurn);
        }

        private void AddArcPoints(Vector3 corner, PolylinePoint next, float radius, bool useDensity,
            int targetPointCount, float pointsPerTurn)
        {
            if (CheckCanAddContinuePoint()) return;

            var prev = LastPoint;

            var tangentA = (corner - prev.point).normalized;
            var tangentB = (next.point - corner).normalized;
            var cross = Vector3.Cross(tangentA, tangentB);
            ...
            if (cross.TaxicabMagnitude() <= 0.001f)
            {
                ...
                var tCenter = ShapesMath.GetLineSegmentProjectionT(prev.point, next.point, corner);
                var tA = Mathf.Clamp01(tCenter - 0.0001f); // nudge to make sharp discontinuity
                var tB = Mathf.Clamp01(tCenter + 0.0001f); // nudge to make sharp discontinuity
                var ppA = prev;
                var ppB = next;
            ...
            var normA = Vector3.Cross(axis, tangentA); // normalized
            var normB = Vector3.Cross(axis, tangentB);
            var cornerDir = (normA + normB).normalized;
            var cornerBDot = Vector3.Dot(cornerDir, normB);
            ...
            if (useDensity)
            {
                var angTurn = Vector3.Angle(normA, normB) / 360f;
        ...
        [Obsolete(
            "This function no longer exists - either use the overload without a color, where the color will match the previous point, or the one with a PolylinePoint endpoint, where the color will blend between previous point and the target point",
            true)]
