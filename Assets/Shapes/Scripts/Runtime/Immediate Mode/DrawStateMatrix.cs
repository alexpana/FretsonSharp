...
namespace Shapes
{
    public static partial class Draw
    {
        ...
        /// <summary>
        ///     The current immediate-mode drawing matrix. Setting this to transform.localToWorldMatrix will make following
        ///     draw calls be relative to that object
        /// </summary>
        ...
        /// <summary>
        ///     using( MatrixScope ){ /*code*/ } lets you modify the matrix within that scope, automatically restoring the
        ///     previous state once you leave the scope
        /// </summary>
        public static MatrixStack MatrixScope => new(Matrix);
        ...
        /// <summary>
        ///     Pushes the current drawing matrix onto the matrix stack. Calling <see cref="Draw.PopMatrix()" /> will restore
        ///     this state
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        {
            get => new(matrix.m03, matrix.m13, matrix.m23);
            set
            {
                matrix.m03 = value.x;
                matrix.m13 = value.y;
        ...
        /// <summary>
        ///     Attempts to get or directly set the rotation of the drawing matrix in world space.
        ///     Note that, while rare, non-orthogonal matrices cannot be represented with a quaternion rotation, so the getter
        ///     might not behave as you expect it to.
        ///     Setting this rotation frequently may be expensive as it will also have to calculate scale in order to retain its
        ...
        /// <summary>
        ///     Gets or sets the 2D angle (in radians) that the X axis is pointing in on the world space 2D XY plane. Note
        ///     that this may have undesired effects if the drawing matrix is not aligned with the Z plane
        /// </summary>
        ...
        /// <summary>
        ///     The world space X axis direction of the current drawing matrix (normalized). If you don't need this
        ///     normalized, or if you know you haven't scaled your matrix, then RightBasis is computationally less expensive
        /// </summary>
        public static Vector3 Right => RightBasis.normalized;

        /// <summary>
        ///     The world space Y axis direction of the current drawing matrix (normalized). If you don't need this
        ///     normalized, or if you know you haven't scaled your matrix, then UpBasis is computationally less expensive
        /// </summary>
        public static Vector3 Up => UpBasis.normalized;

        /// <summary>
        ///     The world space Z axis direction of the current drawing matrix (normalized). If you don't need this
        ///     normalized, or if you know you haven't scaled your matrix, then ForwardBasis is computationally less expensive
        /// </summary>
        public static Vector3 Forward => ForwardBasis.normalized;

        /// <summary>
        ///     The world space X axis basis vector of the current drawing matrix. The magnitude of this vector is the local X
        ///     scale
        /// </summary>
        public static Vector3 RightBasis => matrix.GetColumn(0);

        /// <summary>
        ///     The world space Y axis basis vector of the current drawing matrix. The magnitude of this vector is the local Y
        ///     scale
        /// </summary>
        public static Vector3 UpBasis => matrix.GetColumn(1);

        /// <summary>
        ///     The world space Z axis basis vector of the current drawing matrix. The magnitude of this vector is the local Z
        ///     scale
        /// </summary>
        public static Vector3 ForwardBasis => matrix.GetColumn(2);

        /// <summary>
        ///     Gets or sets the local per-axis scale of the current drawing matrix. This is relatively expensive as it needs
        ///     to calculate the magnitude of each basis vector
        /// </summary>
        public static Vector3 LocalScale
        {
            get => new(RightBasis.magnitude, UpBasis.magnitude, ForwardBasis.magnitude);
            set
            {
                // v.normalized * scale = (v/||v||) * scale = v * scale/||v||
                var xScale = value.x / RightBasis.magnitude;
                var yScale = value.y / UpBasis.magnitude;
                var zScale = value.z / ForwardBasis.magnitude;
        ...
        {
            Rotate(Quaternion.AngleAxis(angle * Mathf.Rad2Deg, axis));
        }
        ...
        {
            var rotMtx = Matrix4x4.Rotate(rotation);
            var scaleX = ((Vector3)m.GetColumn(0)).magnitude;
            var scaleY = ((Vector3)m.GetColumn(1)).magnitude;
            var scaleZ = ((Vector3)m.GetColumn(2)).magnitude;
