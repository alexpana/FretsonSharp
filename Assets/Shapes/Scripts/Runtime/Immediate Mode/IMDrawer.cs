...
namespace Shapes
{
    internal struct IMDrawer : IDisposable
    {
        ...
        private static readonly Dictionary<Material, string[]> matKeywords = new();

        private static string[] GetMaterialKeywords(Material m)
        {
            if (matKeywords.TryGetValue(m, out var kws) == false)
                matKeywords[m] = kws = m.shaderKeywords;
            return kws;
        }

        private readonly MetaMpb metaMpb;
        private readonly ShapeDrawState drawState;
        private readonly Matrix4x4 mtx;
        private readonly bool allowInstancing;
        ...
        public IMDrawer(MetaMpb metaMpb, Material sourceMat, Mesh sourceMesh, int submesh = 0,
            DrawType drawType = DrawType.Shape, bool allowInstancing = true, int textAutoDisposeId = -1)
        {
            mtx = Draw.Matrix;
            ...
            if (DrawCommand.IsAddingDrawCommandsToBuffer)
            {
                ...
                Draw.style.renderState.isTextMaterial =
                    drawType == DrawType.TextPooledPersistent || drawType == DrawType.TextAssetClone;
                ...
                if (metaMpbPrevious != metaMpb && metaMpbPrevious != null && metaMpbPrevious.HasContent)
                    DrawCommand.CurrentWritingCommandBuffer.drawCalls.Add(metaMpbPrevious
                        .ExtractDrawCall()); // finalize previous buffer
                ...
                if (metaMpb.PreAppendCheck(drawState, mtx) == false)
                {
                    ...
                    DrawCommand.CurrentWritingCommandBuffer.drawCalls
                        .Add(metaMpb.ExtractDrawCall()); // finalize previous buffer
                    if (metaMpb.PreAppendCheck(drawState, mtx) ==
                        false) // append again now that the call has been dispatched
            ...
            else
            {
                ...
                ApplyGlobalProperties(drawState
                    .mat); // this will set render state of the material. todo: will this modify the assets? this seems bad
            }
        }
