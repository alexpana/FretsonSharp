...
namespace Shapes
{
    ...
    {
        private bool disposeWhenFullyReleased = false;
        protected bool hasData = false; // used to detect whether or not we can/should generate a mesh on draw
        private bool hasMesh = false; // used to detect if mesh needs to be created on the fly on draw

        protected Mesh mesh;
        protected bool meshDirty = false;

        internal List<DrawCommand> usedByCommands = null;
        public static int ActiveMeshCount { get; private set; }
        ...
        public void Dispose()
        {
            ...
            var hasCommandList = usedByCommands != null;
