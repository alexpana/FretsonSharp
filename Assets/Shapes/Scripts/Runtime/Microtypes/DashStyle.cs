...
namespace Shapes
{
    ...
    {
        public static readonly DashStyle defaultDashStyle = new()
        {
            type = DashType.Basic,
        ...
        public static readonly DashStyle defaultDashStyleRing = new()
        {
            type = DashType.Basic,
        ...
        public static readonly DashStyle defaultDashStyleLine = new()
        {
            type = DashType.Basic,
            space = DashSpace.Relative,
        ...
        [Range(-1f, 1f)] public float shapeModifier;
        ...
        {
            return space == DashSpace.Relative ? thickness * v : v;
        }

        internal float GetNetAbsoluteSize(bool dashed, float thickness)
        {
            return dashed ? GetNet(size, thickness) : 0f;
        }

        internal float GetNetAbsoluteSpacing(bool dashed, float thickness)
        {
            return dashed ? GetNet(spacing, thickness) : 0f;
        }

        /// <summary>
        ///     Creates a dash style where dash lengths are relative to the thickness of the object you're drawing. A dash
        ///     size of 1 means the dash is as long as the shape is thick
        /// </summary>
        /// <param name="type">The type of dash to use (basic, rounded or angled)</param>
        /// <param name="size">
        ///     The size/length of a single dash. A size of 1 is equal to the thickness of the shape you're using
        ///     these dashes on
        /// </param>
        /// <param name="spacing">
        ///     The size/length of the spacing between each dash. A size of 1 is equal to the thickness of the
        ///     shape you're using these dashes on
        /// </param>
        /// <param name="snap">What snapping mode to use, if any</param>
        /// <param name="offset">
        ///     Longitudinal offset for the dashes. A value of 1 is the length of a single dash+space period,
        ///     which means the offset repeats/looks the same at every integer
        /// </param>
        /// <param name="shapeModifier">
        ///     A modifier for the dash styles, which will set the skewness of angled dashes, for instance.
        ///     Valid range: -1 to 1
        /// </param>
        public static DashStyle RelativeDashes(DashType type, float size, float spacing,
            DashSnapping snap = DashSnapping.Off, float offset = 0f, float shapeModifier = 1f)
        {
            return new()
            {
                space = DashSpace.Relative,
        ...
        /// <param name="spacingFraction">
        ///     The fraction of each repeating period that should be space. A value of 0.5f means spaces
        ///     will have the same size as dashes. Valid range: greater than 0, less than 1
        /// </param>
        /// <param name="snap">What snapping mode to use, if any</param>
        /// <param name="offset">
        ///     Longitudinal offset for the dashes. A value of 1 is the length of a single dash+space period,
        ...
        ///     Valid range: -1 to 1
        /// </param>
        public static DashStyle FixedDashCount(DashType type, float count, float spacingFraction = 0.5f,
            DashSnapping snap = DashSnapping.Off, float offset = 0f, float shapeModifier = 1f)
        {
            return new()
            {
        ...
        ///     Valid range: -1 to 1
        /// </param>
        public static DashStyle MeterDashes(DashType type, float size, float spacing,
            DashSnapping snap = DashSnapping.Off, float offset = 0f, float shapeModifier = 1f)
        {
            return new()
            {
