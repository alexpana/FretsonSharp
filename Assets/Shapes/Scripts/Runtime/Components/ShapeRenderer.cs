...
namespace Shapes
{
    ...
    {
        // Properties
        [SerializeField] private ShapesBlendMode blendMode = ShapesBlendMode.Transparent;
        ...
        private Material[]
            instancedMaterials =
                null; // used when pass tags are anything but the default (eg ZTest != Less Equal, or scale offset is set, or a weird blend mode)

        /// <summary>
        ///     Used to mark this mesh as changed. Only required when manually editing the point lists of polygons and
        ///     polylines, otherwise you shouldn't ever need to use this
        /// </summary>
        [NonSerialized] public bool meshOutOfDate = true;
        ...
        private MaterialPropertyBlock Mpb =>
            mpb ?? (mpb = new MaterialPropertyBlock()); // hecking, gosh, I want the C#8 ??= operator

        internal bool IsUsingUniqueMaterials => IsInstanced == false;

        /// <summary>
        ///     The current mesh in use by this shape. Note: Do not directly modify this mesh, it may not update properly or
        ///     your changes will be overwritten or you'll modify assets. It's here for reading purposes only
        /// </summary>
        public Mesh Mesh
        {
            get => mf.sharedMesh;
        ...
        private bool HasGeneratedOrCopyOfMesh => MeshUpdateMode == MeshUpdateMode.SelfGenerated ||
                                                 MeshUpdateMode == MeshUpdateMode.UseAssetCopy;
        ...
        private void OnDestroy()
        {
            ...
            TryDestroyInstancedMaterials(true);
        }

        private void OnDidApplyAnimationProperties()
        {
            UpdateAllMaterialProperties();
            // so this is not great but it works don't judge
        }
        ...
        private void VerifyComponents()
        {
            if (initializedComponents == false)
            {
                ...
                MakeSureComponentExists(ref rnd, out var createdRnd);
            }
...
#if UNITY_EDITOR
        private void UpdateMeshOnUndoRedo()
        {
            UpdateMesh(true);
        }
#endif
        ...
        {
            return ShapesMeshUtils.QuadMesh[HasDetailLevels ? (int)DetailLevel.Medium : 0];
        }
        ...
        private void MakeSureMaterialInstancesAreGood(Material[] sourceMats)
        {
            ...
            {
                return new(sourceMats[index]) { name = sourceMats[index].name + " (instance)" };
            }
...
#if UNITY_EDITOR
        private void UpdateMaterialsEditorMode(Material[] targetMats)
        {
            var needsUpdate = false;
            if (rnd.sharedMaterials.Length != targetMats.Length)
                needsUpdate = true;
            else
                for (var i = 0; i < targetMats.Length; i++)
	    ...
	    /// <summary>
	    ///     Updates the mesh this object is using.
	    ///     If this mesh generates procedural meshes, such as the polygon or polyline, it will regenerate it if force is set to
	    ///     true
	    /// </summary>
	    /// <param name="force"></param>
	    public void UpdateMesh(bool force = false)
        {
            var mode = MeshUpdateMode;
            ...
            var createMesh = Mesh == null || meshOwnerID != id;
	    ...
	    /// <summary>
	    ///     <para>Returns the local space bounds</para>
	    ///     <para>
	    ///         Note: This does not take into account screen space sizing, so it will only behave as you expect when using
	    ///         meters only
	    ///     </para>
	    /// </summary>
	    ...
	    /// <summary>
	    ///     <para>Returns the world space bounds of the local space bounds</para>
	    ///     <para>
	    ///         Note: This does not take into account screen space sizing, so it will only behave as you expect when using
	    ///         meters only
	    ///     </para>
        ...
        {
            var localBounds = GetBounds_Internal();
            ...
            for (var z = -1; z <= 1; z += 2)
            {
                var wPt = tf.TransformPoint(localBounds.center +
        ...
        private void SetIntOnAllInstancedMaterials(int property, int value)
        {
            ...
            {
                UpdateMaterial();
                foreach (var instancedMaterial in instancedMaterials)
        ...
        private protected void SetAllDashValues(DashStyle style, bool dashed, bool matchSpacingToSize, float thickness,
            bool setType, bool now)
        {
            var netDashSize = style.GetNetAbsoluteSize(dashed, thickness);
            if (dashed)
            {
                SetFloat(ShapesMaterialUtils.propDashSpacing,
                    GetNetDashSpacing(style, true, matchSpacingToSize, thickness));
        ...
        private protected float GetNetDashSpacing(DashStyle style, bool dashed, bool matchSpacingToSize,
            float thickness)
        {
            if (matchSpacingToSize && style.space == DashSpace.FixedCount)
                return 0.5f;
            return matchSpacingToSize
                ? style.GetNetAbsoluteSize(dashed, thickness)
                : style.GetNetAbsoluteSpacing(dashed, thickness);
        }
        ...
        private protected void SetColor(int prop, Color value)
        {
            if (ShapeGroup.shapeGroupsInScene > 0)
            {
                ...
                var groups = GetComponentsInParent<ShapeGroup>();
                if (groups != null)
                    foreach (var shapeGroup in groups.Where(g => g.IsEnabled))
        ...
        private bool IsInstanced => UsingDefaultZTests && UsingDefaultMasking && UsingDefaultRenderQueue;

        // render queue offset
        private bool UsingDefaultRenderQueue => renderQueue == DEFAULT_RENDER_QUEUE_AUTO;
        ...
        [SerializeField] private int renderQueue = DEFAULT_RENDER_QUEUE_AUTO;
        ...
        private bool UsingDefaultZTests => zTest == DEFAULT_ZTEST && zOffsetFactor == DEFAULT_ZOFS_FACTOR &&
                                           zOffsetUnits == DEFAULT_ZOFS_UNITS;

        [SerializeField] private CompareFunction zTest = DEFAULT_ZTEST;
        ...
        [SerializeField] private float zOffsetFactor = DEFAULT_ZOFS_FACTOR;
        ...
        [SerializeField] private int zOffsetUnits = DEFAULT_ZOFS_UNITS;
        ...
        [SerializeField] private ColorWriteMask colorMask = DEFAULT_COLOR_MASK;
        ...
        private bool UsingDefaultMasking => stencilComp == DEFAULT_STENCIL_COMP &&
                                            stencilOpPass == DEFAULT_STENCIL_OP &&
                                            stencilRefID == DEFAULT_STENCIL_REF_ID &&
                                            stencilReadMask == DEFAULT_STENCIL_MASK &&
                                            stencilWriteMask == DEFAULT_STENCIL_MASK && colorMask == DEFAULT_COLOR_MASK;

        [SerializeField] private CompareFunction stencilComp = DEFAULT_STENCIL_COMP;
        ...
        [SerializeField] private StencilOp stencilOpPass = DEFAULT_STENCIL_OP;
        ...
        [SerializeField] private byte stencilRefID = DEFAULT_STENCIL_REF_ID;
        ...
        [SerializeField] private byte stencilReadMask = DEFAULT_STENCIL_MASK;
        ...
        [SerializeField] private byte stencilWriteMask = DEFAULT_STENCIL_MASK;
        ...
        public virtual void OnValidate()
        {
            ...
            if (MeshUpdateMode == MeshUpdateMode.SelfGenerated)
                meshOutOfDate =
                    true; // we can't update the mesh in OnValidate because of https://issuetracker.unity3d.com/issues/warning-appears-when-changing-meshfilter-dot-sharedmesh-during-onvalidate
