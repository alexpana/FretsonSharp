...
namespace Shapes
{
    ...
    public class Quad : ShapeRenderer
    {
        ...
        [SerializeField] private QuadColorMode colorMode = QuadColorMode.Single;

        [SerializeField] private Vector3 a = new Vector2(-0.5f, -0.5f);
        [SerializeField] private Vector3 b = new Vector2(-0.5f, 0.5f);
        [SerializeField] private Vector3 c = new Vector2(0.5f, 0.5f);
        [SerializeField] private Vector3 d = new Vector2(0.5f, -0.5f);
        [SerializeField] private bool autoSetD = false;
                ...
                switch (index)
                {
                    ...
                    default:
                        throw new IndexOutOfRangeException(
                            $"Quad only has four vertices, 0 to 3, you tried to access element {index}");
                }
            }
            set
            {
                switch (index)
                {
                    ...
                    default:
                        throw new IndexOutOfRangeException(
                            $"Quad only has four vertices, 0 to 3, you tried to set element {index}");
                }
            }
        }
        ...
        public Vector3 D
        {
            ...
            set
            {
                if (autoSetD)
                    Debug.LogWarning(
                        "tried to set D when auto-set is enabled, you might want to turn off auto-set on this object",
                        gameObject);
                else
        ...
        private protected override void SetAllMaterialProperties()
        {
            ...
            switch (colorMode)
            {
                ...
                case QuadColorMode.Vertical:
                    SetColor(ShapesMaterialUtils.propColor,
                        colorD); // todo: this is a double-assign, it's already set before this, but to the wrong value :c
        ...
        {
            return new[] { ShapesMaterialUtils.matQuad[BlendMode] };
        }
