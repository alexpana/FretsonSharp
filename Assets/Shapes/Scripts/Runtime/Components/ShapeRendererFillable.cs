...
namespace Shapes
{
    ...
    {
        private const string OBSOLETE =
            "Shapes now use the IFillable interface instead of inheriting from ShapeRendererFillable";

        // global color fill gradient shenanigans
        [Obsolete(OBSOLETE, true)] private protected GradientFill fill = new();
        [Obsolete(OBSOLETE, true)] private protected bool useFill;
        [Obsolete(OBSOLETE, true)] private int FillTypeShaderInt => useFill ? (int)fill.type : GradientFill.FILL_NONE;
