...
namespace Shapes
{
    internal static class ShapesExtensions
    {
        ...
        public static IEnumerable<TResult> Zip<T1, T2, T3, TResult>(this IEnumerable<T1> source, IEnumerable<T2> second,
            IEnumerable<T3> third, Func<T1, T2, T3, TResult> func)
        {
        ...
        public static bool HasPrefabOverride(this SerializedProperty property)
        {
            return property != null && property.serializedObject.targetObjects.Length == 1 &&
        ...
        public static bool[] TryGetMultiselectPressedStates(this SerializedProperty prop, int[] indexOverride,
            int entryCount)
        {
            var so = prop.serializedObject;
            if (so.isEditingMultipleObjects)
            {
                var bools = new bool[entryCount];
                foreach (var shapeID in so.targetObjects.Select(obj =>
                             new SerializedObject(obj).FindProperty(prop.propertyPath).GetIntValue(indexOverride)))
                {
                    var index = indexOverride == null ? shapeID : Array.IndexOf(indexOverride, shapeID);
