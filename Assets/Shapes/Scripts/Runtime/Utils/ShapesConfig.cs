...
namespace Shapes
{
    ...
    public class ShapesConfig : ScriptableObject
    {
        ...
        private const float VERY_LORGE_BOUNDS = 1 << 16;

        private static ShapesConfig inst;

        [Tooltip(
            "Whether or not to use HDR color pickers throughout Shapes (This does not affect performance in any way)")]
        ...
        [Tooltip(
            "GPU Instancing in immediate mode drawing means if you render lots of similar shapes consecutively, they will get batched into a single draw call. " +
        ...
        [Tooltip(
            "If this is on, static properties set inside of Draw.Command will apply only within that draw command. " +
