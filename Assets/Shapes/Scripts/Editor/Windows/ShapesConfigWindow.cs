...
{
    // General
    private SerializedProperty useHdrColorPickers;
    private SerializedProperty autoConfigureRenderPipeline;
    private SerializedProperty useImmediateModeInstancing;
    private SerializedProperty pushPopStateInDrawCommands;
    private SerializedProperty polylineDefaultPointsPerTurn;
    private SerializedProperty polylineBezierAngularSumAccuracy;

    // Primitive detail
    private SerializedProperty sphereDetail;
    private SerializedProperty torusDivsMinorMajor;
    private SerializedProperty coneDivs;
    private SerializedProperty cylinderDivs;
    private SerializedProperty capsuleDivs;

    // Primitive Bounds
    private SerializedProperty boundsSizeQuad;
    private SerializedProperty boundsSizeTriangle;
    private SerializedProperty boundsSizeSphere;
    private SerializedProperty boundsSizeTorus;
    private SerializedProperty boundsSizeCuboid;
    private SerializedProperty boundsSizeCone;
    private SerializedProperty boundsSizeCylinder;
    private SerializedProperty boundsSizeCapsule;

    private SerializedProperty FRAG_OUTPUT_V4;
    private SerializedProperty LOCAL_ANTI_ALIASING_QUALITY;
    private SerializedProperty QUAD_INTERPOLATION_QUALITY;
    private SerializedProperty NOOTS_ACROSS_SCREEN;

    private SerializedObject so; // Config SO

    private bool showDetailLevels = false;
    ...
    private static readonly string TOOLTIP_DETAIL_SPHERE =
        "The tessellation of the sphere is based on the icosphere. " +
    ...
    private static readonly string TOOLTIP_DETAIL_TORUS =
        "The tessellation of the torus is based on the number of divisions around the major axis and its minor axis";

    private static readonly string TOOLTIP_DETAIL_CONE =
        "The tessellation of the cone is based on the number of divisions per full circle. The triangle count shown is the number of triangles of the capped cone";

    private static readonly string TOOLTIP_DETAIL_CYLINDER =
        "The tessellation of the cylinder is based on the number of divisions per full circle";

    private static readonly string TOOLTIP_DETAIL_CAPSULE =
        "The tessellation of the capsule is based on the octasphere, where you specify detail level per quadrant. " +
    ...
    private void AdvancedSettings()
    {
        using (ShapesUI.Group)
        {
            ...
            if (ShapesUI.CenteredButton(new GUIContent("Regenerate Shaders & Materials",
                    "Generates all shaders and materials in Shapes")))
                CodegenShaders.GenerateShadersAndMaterials(UnityInfo.GetCurrentRenderPipelineInUse());
            if (ShapesUI.CenteredButton(new GUIContent("Regenerate Draw Overloads",
                    "Regenerates all Draw.X overload functions to DrawOverloads.cs")))
                CodegenDrawOverloads.GenerateDrawOverloadsScript();
            if (ShapesUI.CenteredButton(new GUIContent("Regenerate Component Interfaces",
                    "Regenerates all Shape component interfaces")))
                CodegenInterfaces.Generate();
            if (ShapesUI.CenteredButton(new GUIContent("Regenerate IM meta MPBs",
                    "Regenerates all meta-material property blocks for each shape, based on their shader parameters in the core.cginc files")))
                CodegenMpbs.Generate();
        }
    }

    private void CsharpConfigSettings()
    {
        using (new ShapesUI.AssetEditScope(ShapesConfig.Instance))
        {
            ...
            using (ShapesUI.Group)
            {
                ...
                {
                    EditorGUILayout.PropertyField(pushPopStateInDrawCommands,
                        new GUIContent("Always push/pop in Draw.Command"));
                }
    ...
    private void RenderPipeSettings()
    {
        using (ShapesUI.Group)
        {
            ...
            var rpShaders = ShapesImportState.Instance.currentShaderRP;
            RenderPipeline? rpKeywords = ShapesImportState.TryGetPreprocessorRP(out var rpkw) ? rpkw : null;
            ...
            if (rpShaders != rp)
                EditorGUILayout.HelpBox(
                    "Shaders seem to be compiled to a different render pipeline. You might want to force-set to your render pipeline below",
                    MessageType.Warning);
            ...
            if (rpKeywords.HasValue)
            {
                ...
                if (rpKeywords != rp)
                    EditorGUILayout.HelpBox(
                        "Preprocessor keywords seem to be defined for a different render pipeline. You might want to force-set to your render pipeline below",
                        MessageType.Warning);
            }
            else
            {
                EditorGUILayout.HelpBox(
                    "Keywords are currently in a mixed state. Please force-set to your render pipeline below",
                    MessageType.Error);
            }
    ...
    private void PrimitiveSettings()
    {
        using (ShapesUI.Group)
        {
            ...
            {
                if (showDetailLevels = EditorGUILayout.Foldout(showDetailLevels,
                        new GUIContent("Detail Levels",
                            "Configure the mesh density of each detail level preset, for every type of Shape that can make use of it")))
                    using (new EditorGUI.IndentLevelScope(1))
                    {
                        DrawDetailLevelProperties(sphereDetail, "Sphere", TOOLTIP_DETAIL_SPHERE, 1,
                            p => PrimitiveGenerator.TriangleCountIcosphere(p.intValue));
                        DrawDetailLevelProperties(torusDivsMinorMajor, "Torus", TOOLTIP_DETAIL_TORUS, 3,
                            p => PrimitiveGenerator.TriangleCountTorus(p.vector2IntValue));
                        DrawDetailLevelProperties(coneDivs, "Cone", TOOLTIP_DETAIL_CONE, 3,
                            p => PrimitiveGenerator.TriangleCountCone(p.intValue));
                        DrawDetailLevelProperties(cylinderDivs, "Cylinder", TOOLTIP_DETAIL_CYLINDER, 3,
                            p => PrimitiveGenerator.TriangleCountCylinder(p.intValue));
                        DrawDetailLevelProperties(capsuleDivs, "Capsule", TOOLTIP_DETAIL_CAPSULE, 1,
                            p => PrimitiveGenerator.TriangleCountCapsule(p.intValue));
                    }

                if (showBounds = EditorGUILayout.Foldout(showBounds,
                        new GUIContent("Bounds Sizes", ShapesConfig.TOOLTIP_BOUNDS)))
                    using (new EditorGUI.IndentLevelScope(1))
                    {
                ...
                GUILayout.Space(3);
                if (ShapesUI.CenteredButton(new GUIContent("Apply & regenerate primitives",
                        "Regenerate all primitive mesh assets using the configuration above")))
    ...
    private void DrawDetailLevelProperties(SerializedProperty array, string label, string tooltip, int min,
        Func<SerializedProperty, int> triCount)
    {
    ...
    private void DrawDetailLevelEntry(SerializedProperty prop, DetailLevel detail, int min,
        Func<SerializedProperty, int> triCount)
    {
        var isInt2 = prop.propertyType == SerializedPropertyType.Vector2Int;
            ...
            using (ShapesUI.TempLabelWidth(100))
            {
                using (var chChk = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (chChk.changed)
                    {
                        // clamp
                        if (isInt2)
                            prop.vector2IntValue = new Vector2Int(Mathf.Max(min, prop.vector2IntValue.x),
                                Mathf.Max(min, prop.vector2IntValue.y));
                        else
    ...
    private static Object configShaderAssetInst;
    ...
    private static Object ConfigShaderAssetInst
    {
        get
        {
            if (configShaderAssetInst == null)
                configShaderAssetInst = AssetDatabase.LoadAssetAtPath<Object>(ShapesIO.ConfigShadersPath);
            return configShaderAssetInst;
        }
    }

    private void ShaderSettings()
    {
        using (ShapesUI.Group)
        {
            ...
            using (new ShapesUI.AssetEditScope(ConfigShaderAssetInst))
            {
                using (ShapesUI.TempLabelWidth(160))
                {
                    ...
                    EditorGUILayout.PropertyField(QUAD_INTERPOLATION_QUALITY,
                        new GUIContent("Quad interpolation quality"));
    ...
    private void ApplyShaderSettings(string path)
    {
        var lines = new List<string>();

        void Define(SerializedProperty prop, object value)
        {
            lines.Add($"#define {prop.name} {value}");
        }
