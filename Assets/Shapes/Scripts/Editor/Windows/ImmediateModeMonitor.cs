...
public class ImmediateModeMonitor : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        {
            var suffix = Math.Abs(count) == 1 ? "" : "s";
            GUILayout.Label(string.Format(str, count, suffix));
        }

        GUILayout.Label(
            "• The purpose of this window is primarily to check for leaks - if you see any of these amounts increase indefinitely, something is likely wrong\n\n" +
        ...
        using (ShapesUI.Group)
        {
            ...
            CountSuffixLabel(ShapesTextPool.InstanceElementCount,
                $"{{0}} text element{{1}} in the text cache. Usage: {ShapesTextPool.InstanceElementCountActive}/{ShapesTextPool.InstanceElementCount}");
            ...
            CountSuffixLabel(DrawCommand.cBuffersRendering.Values.Sum(list => list.Count),
                "{0} active command buffer{1}");
        }
        ...
        using (var scroll = new EditorGUILayout.ScrollViewScope(scrollPos, false, false))
        {
            ...
            foreach (var kvp in DrawCommand.cBuffersRendering)
            {
                ...
                {
                    EditorGUILayout.ObjectField(GUIContent.none, kvp.Key, typeof(Camera), true, GUILayout.Width(120));
                    GUILayout.Label($"has {kvp.Value.Count} buffer{(kvp.Value.Count != 1 ? "s" : "")}:");
                }

                using (new EditorGUI.IndentLevelScope())
                {
                    foreach (var cmd in kvp.Value) EditorGUILayout.LabelField("Shapes Cmd " + cmd.id);
                }
            }
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                foreach (var kvp in IMMaterialPool.pool) EditorGUILayout.ObjectField(kvp.Value, typeof(Material), true);
            }
