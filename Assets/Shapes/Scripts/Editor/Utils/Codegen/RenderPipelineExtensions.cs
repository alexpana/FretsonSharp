...
namespace Shapes
{
    ...
    {
        private static string PipelineSubshaderTagValue(this RenderPipeline rp)
        {
            switch (rp)
            {
        ...
        public static IEnumerable<string> GetSubshaderTags(this RenderPipeline rp)
        {
            ...
            if (rp == RenderPipeline.Legacy)
                yield
                    break; // this is due to a bug where SRP sometimes picks the legacy pipeline. Putting it last and without a tag fixes this 
