...
{
    private const string DD_LINE = nameof(DashStyle) + "." + nameof(DashStyle.defaultDashStyleLine);
    private const string DD_RING = nameof(DashStyle) + "." + nameof(DashStyle.defaultDashStyleRing);
    ...
    public static void Generate()
    {
        ...
        var asm = Assembly.Load("ShapesRuntime");
        var allShapes = asm.DefinedTypes.Where(
            type => type.IsAbstract == false && (type.BaseType == typeof(ShapeRenderer) ||
                                                 type.BaseType?.BaseType == typeof(ShapeRenderer))
        ).OrderBy(type => type.FullName);
        var shapesWithInterfaces = allShapes.Where(sh =>
            sh.ImplementedInterfaces.Contains(typeof(IDashable)) ||
            sh.ImplementedInterfaces.Contains(typeof(IFillable)));
        ...
        // write to file
        var path = ShapesIO.RootFolder + "/Scripts/Runtime/Interfaces/GeneratedInterfaceImplementations.cs";
        code.WriteTo(path);
    }
    ...
    private static string GetDefaultDashStyle(TypeInfo type)
    {
        return type.AsType() == typeof(Line) ? DD_LINE : DD_RING;
    }

    private static string GetDashTypeCondition(TypeInfo type)
    {
        return type.AsType() == typeof(Line) ? "Geometry != LineGeometry.Volumetric3D" : "setType: true";
    }


    private static void AppendDashLines(TypeInfo type)
    {
        ...
        Append("}");
        Append(
            $"void SetAllDashValues( bool now ) => SetAllDashValues( dashStyle, Dashed, matchDashSpacingToSize, thickness, {GetDashTypeCondition(type)}, now );");
        Append(
            "float GetNetDashSpacing() => GetNetDashSpacing( dashStyle, dashed, matchDashSpacingToSize, thickness );");
    }


    private static void AppendFillLines()
    {
