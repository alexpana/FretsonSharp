...
namespace Shapes
{
    ...
    {
        private static bool ZWrite(this ShapesBlendMode blendMode)
        {
            return blendMode == ShapesBlendMode.Opaque;
        }

        private static bool AlphaToMask(this ShapesBlendMode blendMode)
        {
            return blendMode == ShapesBlendMode.Opaque;
        }

        private static string RenderType(this ShapesBlendMode blendMode)
        {
            return blendMode == ShapesBlendMode.Opaque ? "TransparentCutout" : "Transparent";
        }

        private static string Queue(this ShapesBlendMode blendMode)
        {
            return blendMode == ShapesBlendMode.Opaque ? "AlphaTest" : "Transparent";
        }

        private static bool HasSpecialBlendMode(this ShapesBlendMode blendMode)
        {
            return blendMode != ShapesBlendMode.Opaque;
        }

        public static string BlendShaderDefine(this ShapesBlendMode blendMode)
        {
            return blendMode.ToString().ToUpper();
        }

        private static string GetShaderBlendMode(this ShapesBlendMode blendMode)
        {
            switch (blendMode)
            {
                ...
                case ShapesBlendMode.Subtractive:
                    return "One One"; // uses blend operation reverse subtract (one*dst-one*src)
                case ShapesBlendMode.LinearBurn:
                    return "One One"; // modifies src before rendering (subtracts 1 in the shader itself)
