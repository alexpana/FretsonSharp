...
namespace Shapes
{
    public class SceneEditGizmos
    {
        ...
        private bool DidCancelAction => Tools.current != Tool.None ||
                                        (Event.current.type == EventType.KeyDown &&
                                         Event.current.keyCode == KeyCode.Escape);

        private void RepaintSceneAndInspector()
        {
        ...
        public void GUIEditButton(string label = "Edit in Scene")
        {
            ...
            {
                var newValue = GUILayout.Toggle(IsEditing, label, GUI.skin.button);
                if (chChk.changed)
