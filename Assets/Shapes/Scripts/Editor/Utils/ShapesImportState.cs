...
namespace Shapes
{
    ...
    {
        [Header("Do not edit this~")] [SerializeField]
        internal RenderPipeline currentShaderRP = RenderPipeline.Legacy;

        private static ShapesImportState instance;

        public static ShapesImportState Instance => instance != null
            ? instance
            : instance = Resources.Load<ShapesImportState>("Shapes Import State");
        ...
        private static void AutoCheckRenderPipeline()
        {
            var rpInUnity = UnityInfo.GetCurrentRenderPipelineInUse();
            var inst = Instance;
            if (inst == null)
            {
            ...
            var valid = true;
            var error = "";
            var rpShaders = Instance.currentShaderRP;
            ...
            if (TryGetPreprocessorRP(out var rpPreproc))
            {
            ...
            if (valid == false)
            {
                var desc = "Shapes detected a mismatch in render pipeline state.\n" +
                ...
                if (EditorUtility.DisplayDialog("Render pipeline mismatch", desc,
                        $"Recompile for {rpInUnity.PrettyName()}", "cancel")) ForceSetRP(rpInUnity);
            }
        }
        ...
        private static void ForceSetRpSecondPass(RenderPipeline targetRP)
        {
            ...
            {
                var msg =
                    "In order for immediate mode drawing to work, URP render data needs the shapes render features. Would you like to open Shapes settings to make sure immediate mode drawing is supported?";
        ...
        private static void MakeSureSampleMaterialsAreValid()
        {
			...
			#endif
#else
            var targetShader = UIAssets.Instance.birpDefaultShader;
#endif

            var changed = false;
            if (ShapesIO.TryMakeAssetsEditable(UIAssets.Instance
                    .sampleMaterials)) // ensures version control allows us to edit
        ...
        private static Color GetMainColor(Material mat)
        {
            ...
            if (mat.HasProperty(ShapesMaterialUtils.propBaseColor))
                return mat.GetColor(ShapesMaterialUtils.propBaseColor);
            return Color.white;
        }
        ...
        private static void SetCurrentKeywords(IEnumerable<string> keywords)
        {
        ...
        {
            var keywords = GetCurrentKeywords();
            var kwURP = keywords.Contains(RenderPipeline.URP.PreprocessorDefineName());
            var kwHDRP = keywords.Contains(RenderPipeline.HDRP.PreprocessorDefineName());
            rp = default;
            if (kwURP && !kwHDRP)
        ...
        private static void SetPreprocessorRpSymbols(RenderPipeline rpTarget)
        {
            Debug.Log($"Setting preprocessor symbols for {rpTarget.PrettyName()}");
            var symbols = GetCurrentKeywords();
            ...
            {
                var on = rp == rpTarget;
                var ppName = rp.PreprocessorDefineName();
            ...
            if (changed && ShapesIO.TryMakeAssetsEditable(ShapesIO
                    .projectSettingsPath)) //Debug.Log( $"Shapes updated your project scripting define symbols since you seem to be using {rpTarget.PrettyName()}, I hope that's okay~" );
