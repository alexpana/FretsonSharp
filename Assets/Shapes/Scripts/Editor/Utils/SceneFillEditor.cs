...
namespace Shapes
{
    public class SceneFillEditor : SceneEditGizmos
    {
        ...
        private readonly SerializedProperty propColorEnd;
        private readonly SerializedProperty propColorStart;
        private readonly SerializedProperty propLinearEnd;
        private readonly SerializedProperty propLinearStart;
        private readonly SerializedProperty propRadialOrigin;
        private readonly SerializedProperty propRadialRadius;
        private readonly SerializedProperty propSpace;
        private readonly SerializedProperty propType;

        private readonly SerializedProperty propUseFill;

        private readonly SerializedObject soMain;

        private readonly SphereBoundsHandle sphereHandle = ShapesHandles.InitSphereHandle();
        ...
        private bool ShowFillControls => propUseFill.boolValue == false || propUseFill.hasMultipleDifferentValues;
        ...
        public bool DoSceneHandles(bool useFill, Object component, ref GradientFill fill, Transform tf)
        {
            ...
            switch (fill.type)
            {
                case FillType.LinearGradient:
                    ...
                    var linStart = fill.GetLinearStartWorld(tf);
                    var linEnd = fill.GetLinearEndWorld(tf);
                    ...
                    var nootLength = 0.1f * HandleUtility.GetHandleSize(Vector3.Lerp(linStart, linEnd, .5f));
                    var lineTangent = (linEnd - linStart).normalized;
                    var lineNormal = Vector3.Cross(camForward, lineTangent).normalized;
                    var nootOffset = lineNormal * nootLength;
                    const int nootCount = 5;
                    for (var i = 0; i < nootCount; i++)
                    {
                        var t = i / (nootCount - 1f);
                        var pos = Vector3.Lerp(linStart, linEnd, t);
                ...
                case FillType.RadialGradient:
                    ...
                    if (isEditing)
                    {
                        using (new Handles.DrawingScope(Matrix4x4.TRS(fill.GetRadialOriginWorld(tf), handleRotation,
                                   Vector3.one)))
                        {
                            ...
                            if (GUI.changed)
                            {
                                var newWorldRadius = sphereHandle.radius;
                        ...
                        {
                            var newOrigin = Handles.PositionHandle(fill.GetRadialOriginWorld(tf), handleRotation);
                            if (GUI.changed)
                            {
        ...
        {
            var changed = soMain.ApplyModifiedProperties(); // this is a little cursed
        ...
        private void PropertyHeader(ref bool changed)
        {
            using (ShapesUI.Horizontal)
            {
                soMain.Update();
                EditorGUILayout.PropertyField(propUseFill,
                    new GUIContent("Fill", "Whether or not to override the color fill of this shape"),
        ...
        private void SpaceField(ref bool changed)
        {
            using (var chChk = new EditorGUI.ChangeCheckScope())
            {
                var targets = propSpace.serializedObject.targetObjects.Select(obj => new SerializedObject(obj))
                    .ToArray();
                var prevSpaceStates = targets.Select(t => (FillSpace)t.FindProperty("fill.space").enumValueIndex)
                    .ToArray();
                ...
                if (chChk.changed)
                {
                    var currentSpace = (FillSpace)propSpace.enumValueIndex;
                    for (var i = 0; i < prevSpaceStates.Length; i++)
                    {
                        var prevSpace = prevSpaceStates[i];
                        if (currentSpace == prevSpace)
                            continue;

                        var propLinStart = targets[i].FindProperty("fill.linearStart");
                        var propLinEnd = targets[i].FindProperty("fill.linearEnd");
                        var propRadOrigin = targets[i].FindProperty("fill.radialOrigin");
                        var propRadRadius = targets[i].FindProperty("fill.radialRadius");
                        var tf = ((Component)targets[i].targetObject).transform;
        ...
        private void FillModeProperties(ref bool changed)
        {
            soMain.Update();
            ...
            else if (propType.enumValueIndex == (int)FillType.RadialGradient)
            {
                ...
                using (ShapesUI.Horizontal)
                {
                    ...
                    EditorGUILayout.PropertyField(propColorEnd, GUIContent.none,
                        GUILayout.Width(ShapesUI.POS_COLOR_FIELD_COLOR_WIDTH));
                }
            }
