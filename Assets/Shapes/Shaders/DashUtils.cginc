...
inline void ApplyDashMask(inout half shape_mask, DashCoordinates dashCoords, half coordAcross, int type,
                          half dashModifier)
{
    ...
    else
    {
        ...
        if (type == DASH_TYPE_ROUND)
        {
            ...
            half maskFill = saturate((dashSdf - 1 / dashPerThickness) / fwidth(dashSdf) + 0.5);
            // 0.5 offset to center on pixel bounds
