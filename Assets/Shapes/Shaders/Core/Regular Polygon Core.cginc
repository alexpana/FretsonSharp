...
VertexOutput vert(VertexInput v)
{
    ...
    if (hollow)
    {
        ...
        LineWidthData widthDataThickness = GetScreenSpaceWidthDataSimple(
            OBJ_ORIGIN, CAM_RIGHT, thickness, thicknessSpace);
        half thicknessRadius = widthDataThickness.thicknessMeters / 2;
        o.IP_pxCoverage = widthDataThickness.thicknessPixelsTarget;
        // todo: this isn't properly handling coordinate scaling yet
...
FRAG_OUTPUT_V4 frag(VertexOutput i) : SV_Target
{
    ...
    if (IsBetween(tRadial, -1, 2) && IsDashed() && PROP(_Hollow) > 0)
    {
        ...
        DashCoordinates dashData = GetDashCoordinates(dash, dist, distTotal,
                                                      (1 - i.IP_innerRadiusFraction) * i.IP_uniformScale,
                                                      i.IP_pxPerMeter);
