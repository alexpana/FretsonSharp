...
{
    half3 xAxis = (b - a) / abDistance;
    // we don't calculate abDist in here as it's already needed outside of this function
...
inline void GetPerimeterDistance(VertexOutput i, out float dist, out float distTotal)
{
    ...
    if (roundness > 0)
    {
        ...
        {
            // >= here to include fully rounded sign == 1 case
...
inline half GetAngularMask(VertexOutput i, half tRadial, out float tAngular)
{
    ...
    if (IsDashed() && PROP(_Hollow) > 0)
    {
        ...
        DashCoordinates dashData = GetDashCoordinates(dash, dist, distTotal, i.IP_HALF_THICKNESS * 2 * i.IP_inradius,
                                                      i.IP_pxPerMeter);
