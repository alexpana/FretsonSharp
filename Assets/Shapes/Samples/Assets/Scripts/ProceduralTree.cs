...
namespace Shapes
{
    ...
    {
        [Header("Line Style")] [Range(0f, 0.1f)]
        ...
        private readonly Queue<Matrix4x4> mtxQueue = new(); // queue of pending branches to draw
        ...
        private void BranchFrom(Matrix4x4 mtx)
        {
            ...
            Draw.Matrix = mtx;
            var lineLength = Mathf.Lerp(branchLengthMin, branchLengthMax, Random.value); // random branch length
            var offset = new Vector3(0, lineLength, 0); // offset along the local Y axis
            ...
            var branchCount = Random.Range(branchesMin, branchesMax + 1);
            for (var i = 0; i < branchCount; i++)
                using (Draw.MatrixScope)
                {
                    // saves the current matrix state, and restores it at the end of this scope
                    var angDeviation =
                        Mathf.Lerp(-maxAngDeviation, maxAngDeviation,
                            ShapesMath.RandomGaussian()); // random angular deviation
                    if (use3D)
                        Draw.Rotate(angDeviation,
                            ShapesMath.GetRandomPerpendicularVector(Vector3
                                .up)); // rotates the current drawing matrix on a random axis
                    else
