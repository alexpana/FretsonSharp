...
namespace Shapes
{
    ...
    public class Decayer
    {
        ...
        public void Update()
        {
            ...
            var tEval = curve.keys.Length > 0 ? curve.Evaluate(1f - t) : t;
    ...
    public class FpsController : ImmediateModeShapeDrawer
    {
        ...
        [Header("Player Movement")] [Range(0.8f, 1f)]
        public float smoof = 0.99f;

        public float moveSpeed = 1f;
        public float lookSensitivity = 1f;

        [Header("Sidebar Style")] [Range(0f, ShapesMath.TAU / 2)]
        ...
        [Header("Animation")] [Range(0f, 0.3f)]
        ...
        private void Update()
        {
            ...
            if (InputFocus)
            {
                ...
                if (Input.GetMouseButtonDown(0) && ammoBar.HasBulletsLeft)
                {
                    ...
                    if (Physics.Raycast(ray, out var hit) && hit.collider.gameObject.name == "Enemy")
                        crosshair.FireHit();
                }
        ...
        public override void DrawShapes(Camera cam)
        {
            ...
            using (Draw.Command(cam))
            {
                ...
                var radiusPunched = ammoBarRadius + fireSidebarRadiusPunchAmount * crosshair.fireDecayer.value;
        ...
        public static void DrawRoundedArcOutline(Vector2 origin, float radius, float thickness, float outlineThickness,
            float angStart, float angEnd)
        {
            ...
            // rounded caps
            var originBottom = origin + ShapesMath.AngToDir(angStart) * radius;
            var originTop = origin + ShapesMath.AngToDir(angEnd) * radius;
        ...
        {
            var shakeVal = ShapesMath.Frac(Time.time * speed);
            var shakeX = shakeAnimX.Evaluate(shakeVal);
            var shakeY = shakeAnimY.Evaluate(shakeVal);
