...
namespace Shapes
{
    public class Crosshair : MonoBehaviour
    {
        ...
        public Decayer fireDecayer = new();
        public Decayer hitDecayer = new();
        ...
        public void DrawCrosshair()
        {
            ...
            void DrawCross(Vector2[] dirs, float radInner, float radOuter, float thickness, float radialOffset,
                Color color)
            {
                foreach (var dir in dirs)
                {
                    var a = dir * (radInner + radialOffset);
                    var b = dir * (radOuter + radialOffset);
                    Draw.Line(a, b, thickness, LineEndCap.Round, color);
                }
            }

            // draw things
            var thicc = crosshairCrossThickness * Mathf.Lerp(1f, scaleFire, fireDecayer.t);
            DrawCross(dirsOrtho, crosshairCrossInnerRad, crosshairCrossOuterRad, thicc, fireDecayer.value, Color.white);
            DrawCross(dirsDiag, crosshairHitCrossInnerRad, crosshairHitCrossOuterRad, crosshairHitCrossThickness,
                hitDecayer.valueInv, new Color(1, 0, 0, hitDecayer.t));
        }
    }
}